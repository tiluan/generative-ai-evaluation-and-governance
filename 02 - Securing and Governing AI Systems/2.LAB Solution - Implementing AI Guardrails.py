...
# MAGIC %md
# MAGIC ##Tarefa 1: Implemente controles baseados em LLM com `Llama Guard`
...
# MAGIC 3. **Implante o modelo** em um endpoint de Serviço de Modelos.
# MAGIC
# MAGIC Ao integrar o endpoint do Model Serving em seu próprio aplicativo, você ganha a flexibilidade de especificar suas próprias políticas para detectar e impedir vários tipos de conteúdo. Isso garante que seu aplicativo mantenha um ambiente seguro para os usuários.
# MAGIC
...
def query_llamaguard(chat, unsafe_categories = DEFAULT_UNSAFE_CATEGORY):
    try:
        ...
        # Gerar exceção se houver um erro ao consultar o modelo do LlamaGuard
...
# Query de Llama Guard para vários bate-papos
...
def parse_category(code, taxonomy):
    ...
    # Retorne a descrição do código fornecido ou uma mensagem padrão se o código não for encontrado
