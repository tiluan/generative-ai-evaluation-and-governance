...
# MAGIC
# MAGIC 1. Criar um benchmarkl dataset personalizado específico para o caso de uso
...
# MAGIC %md
# MAGIC Pergunta: Quais são algumas estratégias para avaliar seu dataset de benchmark gerado personalizadamente? Por exemplo:
...
# MAGIC
# MAGIC Lembre-se de que a função `mlflow.evaluate` aceita os seguintes parâmetros para esse caso de uso:
# MAGIC
# MAGIC * Um modelo LLM
...
# MAGIC
# MAGIC Em vez de usar o dataset de benchmark genérico como na demonstração, sua tarefa é **Computar métricas ROUGE usando os dados de benchmark específicos de cada caso que acabamos de criar.**
# MAGIC
...
# MAGIC %md
# MAGIC Em seguida, utilize a função anterior e `mlflow.evaluate` para realizar a `text-summarization` avaliação.
...
# MLflow com `evaluate` com uma função personalizada
results = mlflow.evaluate(
    query_iteration,                      # função iterativa acima
    eval_data,                            # eval DF
    targets="ground_truth",               # coluna com saída esperada ou "boa"
...
humor_example_score_2 = mlflow.metrics.genai.EvaluationExample(
    ...
    score=4, # pontuação de humor atribuído à saída
    justification=(
        "The joke is clever and unexpected, resulting in genuine amusement and laughter. It demonstrates wit and creativity, making it highly enjoyable."   # Justificativa para a pontuação atribuída
    ),
)
...
benchmark_comparison_results = mlflow.evaluate(
    model="endpoints:/databricks-meta-llama-3-3-70b-instruct", # Modelo usado para avaliação
    data=humor_data,                                          # Dados para avaliação
    targets="ground_truth",                                  # Coluna com os dados do ground truth
    model_type="text-summarization",                         # Tipo de modelo ou tarefa
    extra_metrics=[comparison_humor_metric],                 # Métrica personalizada para avaliar o profissionalismo
)
